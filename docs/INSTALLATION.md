...
#### Default: HomeWizard P1

The ready-to-import integrated flow is built around the Home Assistant entity names used by the **HomeWizard P1** setup:
...
When those six entities exist, the base flow is **P1-ready out of the box**. Leave the six Node-RED **Events: state** nodes unchanged.

#### Other P1 / energy-meter integrations

For another three-phase meter in Home Assistant, use:
...
1. Copy the adapter to `/config/packages/zendure_dlb_p1_adapter.yaml`.
2. Enable Home Assistant packages if needed:
   ...
   Add `packages:` to an existing `homeassistant:` block rather than creating a duplicate block.
3. In the adapter file, replace only the six `sensor.REPLACE_WITH_...` source entity IDs with the meter's actual L1/L2/L3 current and voltage sensors.
4. Check configuration and restart Home Assistant.
5. Verify the adapter has created all six canonical `sensor.p1_meter_...` entities and that they update.
6. Confirm current is in amperes and voltage in volts. Verify sign/direction with a known load: increasing grid import/load on a phase should increase its positive current reading.
7. Leave the DLB P1 nodes in Node-RED on the canonical entity IDs.

The adapter changes only the interface names presented to DLB; it does not modify or replace the original P1 integration. Full details are in `code/adapters/README.md`.
