...
3. `apply_gast777_dlb_adapter.py` — repeatable future-version patcher.
4. `20260701-NL-Gast777-DLB-Adapter-v1.0-TEST.json` — disabled reference tab only.
...
1. Download/export the new unmodified Gast777 JSON.
2. Run:

   `python apply_gast777_dlb_adapter.py NEW_GAST777.json NEW_GAST777_WITH_DLB_ADAPTER.json`

3. The patcher searches semantically for `POST Request handling` and its stable final return marker.
4. It refuses to continue when that handover point or the expected three POST routes cannot be found. This fail-closed behaviour prevents silently producing a broken flow after a major Gast777 redesign.
5. Import the patched output and test it before removing the previous version.

You can also upload the new Gast777 JSON here; the same patcher can be applied and the resulting node differences reviewed.
