...
## P1 adapter freshness

The normal HomeWizard P1 Dongle path uses the canonical Home Assistant P1 current entities directly. For another meter, adapter v1.1 exposes the original current source through `dlb_source_entity_id`. DLB v2.6.1 uses that original entity for its targeted `last_reported` freshness read while keeping the canonical adapter entity as the normal Node-RED Events: state interface. This fixes false stale/fail-safe behavior caused by checking the template mirror's timestamp.

The controller deliberately does not create a synthetic periodic heartbeat. If a source integration itself does not report repeated/unchanged measurements into Home Assistant, that must be solved at the source or with a genuine source-provided heartbeat.
